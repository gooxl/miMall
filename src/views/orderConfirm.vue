...
export default {
  ...
  data(){
    return{
      ...
      showDelModal:false,  //删除确认弹框 
      showEditModal:false,  //新增确认弹框
      checkIndex:0  //当前收货地址的下标
    }
  },
  methods:{
    getAddressList(){
    ...
    getCartList(){
      this.axios.get('/carts').then(res=>{
        let list=res.cartProductVoList; //购物车中所有商品数据
        this.cartTotalPrice=res.cartTotalPrice; //商品总额
        this.cartList=list.filter(item=>item.productSelected)//过滤出选中状态的商品
        this.cartList.map(item=>{  //遍历选中的商品
          this.count+=item.quantity;  //累计每件商品的数量
        })
      })
    },
    //打开删除地址弹框
    delAddress(item){ 
      this.checkedItem=item; //获取当前地址的对象
      this.userAction=2;//将用户操作赋值为2，0：新增，1:编辑，2：删除
      this.showDelModal=true; //显示删除确认弹框
    },
    //打开添加地址弹框
    openAddressModal(){
      this.userAction=0;//将用户操作赋值为2，0：新增，1:编辑，2：删除
      this.checkedItem={}; //将当前地址对象设为空
      this.showEditModal=true; //打开添加地址弹框
    },
    //打开编辑地址弹框
    editAddressModal(item){
      this.userAction=1;//将用户操作赋值为2，0：新增，1:编辑，2：删除
      this.checkedItem=item; //将当前地址对象设为空
      this.showEditModal=true; //打开编辑地址弹框
    },
    //地址删除、编辑、新增功能
    submitAddress(){ //将此方法传绑定到弹框确认按钮
      let{checkedItem,userAction}=this;  //解构出商品对象和用户操作
      let method,url,params={}; //请求方法和接口
      //按用户操作调用相应请求
      ...
      if(userAction==0||userAction==1){ //只有2：删除不需要参数
        // 解构出参数
      ...
      //动态获取请求方法 即this.axios.get('/接口')
      this.axios[method](url,params).then(res=>{
        this.closeModal();
        this.getAddressList(); //操作完成后获取最新地址列表
        this.$message.success('操作成功！')
      }); 
    },
    closeModal(){ //关闭弹框方法
      this.checkedItem={}; //将当前地址对象设为空
      this.userAction='';//将用户操作赋值为空，0：新增，1:编辑，2：删除
      this.showDelModal=false; //关闭删除地址弹框
      this.showEditModal=false; //关闭新增地址弹框
    },
    // 提交订单
    orderSubmit(){
      //先判断有没有收货地址
      let index=this.addressList[this.checkIndex] //当前地址列表中选中的地址下标
      if(!index){//如果下标为空，说明没有选中地址
        this.$message.error('请选择收货地址')
        return;
      }
      this.axios.post('/orders',{
        shippingId:index.id  //接口根据当前收货地址来创建订单号
      }).then(res=>{
        this.$router.push({ //下单成功后跳转至支付页面，并传递接口返回的订单号
          path:'/order/pay',
          query:{ 
            orderNo:res.orderNo //query参数传递返回的订单号
          }
        })
      })
    }
